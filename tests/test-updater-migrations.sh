...
assert_eq "27" "${#PLANNED_MIGRATIONS[@]}" "0.1.0 should plan every post-0.1 migration"
assert_eq "0.1.0_to_0.2.0" "${PLANNED_MIGRATIONS[0]}" "first planned migration from 0.1.0"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[26]}" "last planned migration from 0.1.0"
build_migration_plan "0.4.5"
assert_eq "19" "${#PLANNED_MIGRATIONS[@]}" "0.4.5 should plan bookkeeping, shutdown safety steps, and later documentation/history steps"
assert_eq "0.4.5_to_0.4.6" "${PLANNED_MIGRATIONS[0]}" "0.4.5 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[18]}" "0.4.5 final planned migration"
build_migration_plan "0.4.7"
assert_eq "17" "${#PLANNED_MIGRATIONS[@]}" "0.4.7 should plan backend-delay, frontend cleanup, stabilization, browser-neutral container-stop, Tailscale firewall migrations, UFW disable parity migration, PWM cleanup, and later history steps"
assert_eq "0.4.7_to_0.4.8" "${PLANNED_MIGRATIONS[0]}" "0.4.7 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[16]}" "0.4.7 final planned migration"
build_migration_plan "0.4.8"
assert_eq "16" "${#PLANNED_MIGRATIONS[@]}" "0.4.8 should plan frontend experiments, stabilization, browser-neutral container-stop, Tailscale firewall migrations, UFW disable parity migration, PWM cleanup, and later history steps"
assert_eq "0.4.8_to_0.4.9" "${PLANNED_MIGRATIONS[0]}" "0.4.8 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[15]}" "0.4.8 final planned migration"
build_migration_plan "0.4.9"
assert_eq "15" "${#PLANNED_MIGRATIONS[@]}" "0.4.9 should plan cache-bust, stabilization, browser-neutral container-stop, Tailscale firewall migrations, UFW disable parity migration, PWM cleanup, and later history steps"
assert_eq "0.4.9_to_0.4.10" "${PLANNED_MIGRATIONS[0]}" "0.4.9 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[14]}" "0.4.9 final planned migration"
build_migration_plan "0.4.10"
assert_eq "14" "${#PLANNED_MIGRATIONS[@]}" "0.4.10 should plan stabilization, browser-neutral container-stop, Tailscale firewall migrations, UFW disable parity migration, PWM cleanup, and later history steps"
assert_eq "0.4.10_to_0.4.11" "${PLANNED_MIGRATIONS[0]}" "0.4.10 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[13]}" "0.4.10 final planned migration"
build_migration_plan "0.4.11"
assert_eq "13" "${#PLANNED_MIGRATIONS[@]}" "0.4.11 should plan browser-neutral container-stop, Tailscale firewall migrations, UFW disable parity migration, PWM cleanup, and later history steps"
assert_eq "0.4.11_to_0.4.12" "${PLANNED_MIGRATIONS[0]}" "0.4.11 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[12]}" "0.4.11 final planned migration"
build_migration_plan "0.4.12"
assert_eq "12" "${#PLANNED_MIGRATIONS[@]}" "0.4.12 should plan documentation, installer-only history, Tailscale firewall migrations, UFW disable parity migration, PWM cleanup, and later history steps"
assert_eq "0.4.12_to_0.4.13" "${PLANNED_MIGRATIONS[0]}" "0.4.12 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[11]}" "0.4.12 final planned migration"
build_migration_plan "0.4.13"
assert_eq "11" "${#PLANNED_MIGRATIONS[@]}" "0.4.13 should plan installer-only history, Tailscale firewall migrations, UFW disable parity migration, PWM cleanup, and later history steps"
assert_eq "0.4.13_to_0.4.14" "${PLANNED_MIGRATIONS[0]}" "0.4.13 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[10]}" "0.4.13 final planned migration"
build_migration_plan "0.4.14"
assert_eq "10" "${#PLANNED_MIGRATIONS[@]}" "0.4.14 should plan installer-only history, Tailscale firewall migrations, UFW disable parity migration, PWM cleanup, and later history steps"
assert_eq "0.4.14_to_0.4.15" "${PLANNED_MIGRATIONS[0]}" "0.4.14 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[9]}" "0.4.14 final planned migration"
build_migration_plan "0.4.15"
assert_eq "9" "${#PLANNED_MIGRATIONS[@]}" "0.4.15 should plan target-scoped SSD busy cleanup, Tailscale firewall migrations, UFW disable parity migration, PWM cleanup, and later history steps"
assert_eq "0.4.15_to_0.4.16" "${PLANNED_MIGRATIONS[0]}" "0.4.15 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[8]}" "0.4.15 final planned migration"
build_migration_plan "0.4.16"
assert_eq "8" "${#PLANNED_MIGRATIONS[@]}" "0.4.16 should plan the 0.4.17, 0.4.18, 0.5.0, 0.5.1, 0.5.2, 0.5.3, 0.5.4, and 0.5.5 history steps"
assert_eq "0.4.16_to_0.4.17" "${PLANNED_MIGRATIONS[0]}" "0.4.16 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[7]}" "0.4.16 final planned migration"
build_migration_plan "0.4.17"
assert_eq "7" "${#PLANNED_MIGRATIONS[@]}" "0.4.17 should plan the 0.4.18, 0.5.0, 0.5.1, 0.5.2, 0.5.3, 0.5.4, and 0.5.5 history steps"
assert_eq "0.4.17_to_0.4.18" "${PLANNED_MIGRATIONS[0]}" "0.4.17 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[6]}" "0.4.17 final planned migration"
build_migration_plan "0.4.18"
assert_eq "6" "${#PLANNED_MIGRATIONS[@]}" "0.4.18 should plan the 0.5.0, 0.5.1, 0.5.2, 0.5.3, 0.5.4, and 0.5.5 history steps"
assert_eq "0.4.18_to_0.5.0" "${PLANNED_MIGRATIONS[0]}" "0.4.18 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[5]}" "0.4.18 final planned migration"
build_migration_plan "0.5.0"
assert_eq "5" "${#PLANNED_MIGRATIONS[@]}" "0.5.0 should plan both Umbrel networking simplification steps, PWM cleanup, and later history steps"
assert_eq "0.5.0_to_0.5.1" "${PLANNED_MIGRATIONS[0]}" "0.5.0 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[4]}" "0.5.0 final planned migration"
build_migration_plan "0.5.1"
assert_eq "4" "${#PLANNED_MIGRATIONS[@]}" "0.5.1 should plan the UFW disable networking simplification step, PWM cleanup, and later history steps"
assert_eq "0.5.1_to_0.5.2" "${PLANNED_MIGRATIONS[0]}" "0.5.1 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[3]}" "0.5.1 final planned migration"
build_migration_plan "0.5.2"
assert_eq "3" "${#PLANNED_MIGRATIONS[@]}" "0.5.2 should plan the PWM cleanup migration plus later history steps"
assert_eq "0.5.2_to_0.5.3" "${PLANNED_MIGRATIONS[0]}" "0.5.2 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[2]}" "0.5.2 final planned migration"
build_migration_plan "0.5.3"
assert_eq "2" "${#PLANNED_MIGRATIONS[@]}" "0.5.3 should plan the later history steps"
assert_eq "0.5.3_to_0.5.4" "${PLANNED_MIGRATIONS[0]}" "0.5.3 first planned migration"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[1]}" "0.5.3 final planned migration"
build_migration_plan "0.5.4"
assert_eq "1" "${#PLANNED_MIGRATIONS[@]}" "0.5.4 should plan the latest helper-script history step"
assert_eq "0.5.4_to_0.5.5" "${PLANNED_MIGRATIONS[0]}" "0.5.4 final planned migration"
build_migration_plan "0.5.5"
assert_eq "0" "${#PLANNED_MIGRATIONS[@]}" "0.5.5 should plan no migrations"
